...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Make sure category buttons are always visible
  ...
  categoryButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      // Filter news cards only (exclude category buttons)
      newsCards.forEach((card) => {
        // Skip if this is a category button
        if (card.classList.contains("category-btn")) {
          return;
        }
      ...
      // Double check that all category buttons remain visible
      categoryButtons.forEach((btn) => {
        btn.style.display = "inline-block";
        btn.style.visibility = "visible";
        btn.style.opacity = "1";
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", function (e) {
      ...
      if (email && isValidEmail(email)) {
        ...
        showNotification(
          "Đăng ký thành công! Cảm ơn bạn đã quan tâm đến VUIDOKAN.",
          "success"
        );
      } else {
  ...
  document
    .querySelectorAll(".news-card p, .featured-news-card p")
    .forEach((content) => {
  ...
  function addSocialSharing() {
    ...
    newsCards.forEach((card) => {
      const title =
        card.querySelector("h3, h4")?.textContent || "Tin tức từ VUIDOKAN";
  ...
  if (searchInput) {
    ...
    searchInput.addEventListener("input", function () {
      ...
      searchTimeout = setTimeout(() => {
        ...
        newsCards.forEach((card) => {
          const title =
            card.querySelector("h3, h4")?.textContent.toLowerCase() || "";
          const content =
        ...
        const visibleCards = document.querySelectorAll(
          '.news-card:not([style*="none"]), .featured-news-card:not([style*="none"])'
        );
        if (visibleCards.length === 0 && query !== "") {
          showNotification(
            'Không tìm thấy tin tức phù hợp với từ khóa "' + query + '"',
            "info"
          );
        }
      }, 300);
